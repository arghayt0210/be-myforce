...
export const createAchievement = async (
  ...
  try {
    ...
    if (files?.length > 10) {
        return next(
          new ErrorHandler(400, 'Maximum 10 files are allowed')
        );
    }
    ...
    if (videoFiles.length > 1) {
      return next(
        new ErrorHandler(400, 'Only one video is allowed per achievement')
      );
    }

    // Check if all interests exist in user's interest list first
    const user = await User.findById(req.user?._id as Types.ObjectId);
    const invalidInterests = result.data.interests.filter(interest => 
    ...
    if (invalidInterests.length > 0) {
      return next(
        new ErrorHandler(400, 'Some interests are not in user\'s interest list')
      );
    }
